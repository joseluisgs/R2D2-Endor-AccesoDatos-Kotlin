...
fun MedicionDto.Companion.loadFromCsvFile(csvFile: File): List<MedicionDto> {
...
fun MedicionDto.toModel(): Medicion {
    return Medicion(
        id = this.id,
        fecha = this.fecha,
        tipo = this.tipo,
...
fun MedicionDto.Companion.loadFromXmlFile(xmlFile: File): List<MedicionDto> {
...
private fun Element.toDto(): MedicionDto {
    /*        <resource>
    ...
    return MedicionDto(
        ...
        fecha = this.getChild("date").value.parseZonedDateTime()
    )
}
